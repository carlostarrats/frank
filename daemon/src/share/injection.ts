...
export async function prepareBundle(opts: PrepareBundleOptions): Promise<PreparedBundle> {
  ...
  // Write overlay source into public/ (framework-specific public dir varies;
  // SvelteKit uses static/ and Astro/Remix still use public/. Hard-coding
  // public/ works for Next/Vite/Astro/Remix; SvelteKit handled below.)
  const publicDirName = framework === 'sveltekit' ? 'static' : 'public';
  const overlayDestPath = path.join(workingDir, publicDirName, 'frank-overlay.js');
